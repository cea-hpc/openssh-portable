...
# No indentation for simpler patch generation - start
rm -rf ${COPY} ${COPY}.1 ${COPY}.2 ${COPY}.dd ${COPY}.dd2
mkdir ${COPY}.dd

verbose "$tid: lls (extra_channels: $N)"
printf "lcd ${OBJ}\nlls\n" | ${SFTP} -n $N -D ${SFTPSERVER} 2>&1 | \
...
echo "!echo hi there" | ${SFTP} -n $N -D ${SFTPSERVER} 2>&1 | \
...
verbose "$tid: cp ((extra_channels: $N)"
rm -f ${COPY}.2
echo "cp ${COPY}.1 ${COPY}.2" | ${SFTP} -n $N -D ${SFTPSERVER} >/dev/null 2>&1 || fail "cp failed"
...
# No indentation for simpler patch generation - end
done
